...
// 季節情報データベース（最小限）
const SEASONAL_INFO = {
    "1-2月": "北風強、ホエールウォッチング、与那国ハンマー",
    "3-5月": "過ごしやすい、マンタシーズン開始",
    "6-8月": "ベストシーズン、ジンベエザメ、台風注意",
    "9-11月": "台風後抜群透明度、秋マンタ",
    "12月": "北風強いが穴場、料金安め"
};
...
function getCurrentSeasonInfo(month) {
    if (month >= 1 && month <= 2) {
        return SEASONAL_INFO["1-2月"];
    } else if (month >= 3 && month <= 5) {
        return SEASONAL_INFO["3-5月"];
    } else if (month >= 6 && month <= 8) {
        return SEASONAL_INFO["6-8月"];
    } else if (month >= 9 && month <= 11) {
        return SEASONAL_INFO["9-11月"];
    } else {
        return SEASONAL_INFO["12月"];
    }
}
